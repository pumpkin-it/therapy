...
router.patch('/:id', auth, (req, res) => {
  db.prepare('UPDATE case_notes SET note = ? WHERE id = ?').run(req.body.note, req.params.id);
  res.json(db.prepare('SELECT * FROM case_notes WHERE id = ?').get(req.params.id));
});

router.delete('/:id', auth, (req, res) => {
  db.prepare('DELETE FROM case_notes WHERE id = ?').run(req.params.id);
  res.status(204).send();
});
