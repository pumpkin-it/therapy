...
router.get('/', auth, (req, res) => {
  ...
  const items = ids.length
    ? db.prepare(`
        SELECT ai.*, s.name AS service_name
...
router.get('/:id', auth, (req, res) => {
  ...
  appt.items = db.prepare(`
    SELECT ai.*, s.name AS service_name FROM appointment_items ai
...
router.post('/', auth, (req, res) => {
  const { practitioner_id, client_id, location, title, start_time, end_time, notes, items = [] } = req.body;
  const result = db.prepare(`
    INSERT INTO appointments (practitioner_id, client_id, location, title, start_time, end_time, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(practitioner_id, client_id, location||null, title||null, start_time, end_time, notes||null);
  ...
  const appt = db.prepare(`
    ...
    FROM appointments a JOIN clients c ON c.id=a.client_id JOIN practitioners p ON p.id=a.practitioner_id
...
router.patch('/:id', auth, (req, res) => {
  const { practitioner_id, client_id, location, title, start_time, end_time, notes, status, items } = req.body;
  db.prepare(`
    UPDATE appointments SET practitioner_id=?, client_id=?, location=?, title=?, start_time=?, end_time=?, notes=?, status=? WHERE id=?
  `).run(practitioner_id, client_id, location||null, title||null, start_time, end_time, notes||null, status||'scheduled', req.params.id);
