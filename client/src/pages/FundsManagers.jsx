  ...
  return (
    <Modal title={fm ? 'Edit Funder' : 'Add Funder'} onClose={onClose}>
      <div className="space-y-3">
        ...
        <Input label="Phone" value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="Optional" />
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <table className="min-w-full divide-y divide-gray-100">
          <thead className="bg-gray-50">
            <tr>
              {['Name', 'Invoice email', 'Phone', ''].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {fms.length === 0 && (
              <tr><td colSpan={4} className="py-12 text-center text-gray-400">No funders added yet</td></tr>
            )}
            {fms.map(fm => (
